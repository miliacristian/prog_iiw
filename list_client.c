...
int close_connection_list(struct temp_buffer temp_buff,int *seq_to_send,struct window_snd_buf*win_buf_snd,int sockfd,struct sockaddr_in serv_addr,socklen_t len,int *window_base_snd,int *window_base_rcv,int *pkt_fly,int W,int *byte_written,double loss_prob,struct shm_snd*shm_snd){
...
int  wait_for_fin_list(struct temp_buffer temp_buff,struct window_snd_buf*win_buf_snd,int sockfd,struct sockaddr_in serv_addr,socklen_t len,int *window_base_snd,int *window_base_rcv,int *pkt_fly,int W,int *byte_written,double loss_prob,struct shm_snd*shm_snd){
                ...
                if (temp_buff.command == DATA) {
                    ...
                    if (*byte_written==dimension) {
                        wait_for_fin_list(temp_buff, win_buf_snd, sockfd, serv_addr, len, window_base_snd,
                                          window_base_rcv, pkt_fly, W, byte_written, loss_prob,shm_snd->shm);
                        return *byte_written;
                    }
                }
                else{
...
int wait_for_list_dimension(int sockfd, struct sockaddr_in serv_addr, socklen_t  len, int *byte_written , int *seq_to_send , int *window_base_snd , int *window_base_rcv, int W, int *pkt_fly , struct temp_buffer temp_buff ,struct window_rcv_buf *win_buf_rcv,struct window_snd_buf *win_buf_snd,struct shm_snd*shm_snd) {
    errno = 0;
    int dimension=0;
    double loss_prob = param_client.loss_prob;
    char*list,*first;
    strcpy(temp_buff.payload, "list");
    send_message_in_window(sockfd, &serv_addr, len, temp_buff, win_buf_snd, temp_buff.payload,LIST, seq_to_send,loss_prob, W, pkt_fly,shm_snd->shm);//manda messaggio get
    alarm(TIMEOUT);
    while (1) {
        if (recvfrom(sockfd, &temp_buff, sizeof(struct temp_buffer), 0, (struct sockaddr *) &serv_addr, &len) !=
            ...
            if (temp_buff.seq == NOT_A_PKT && temp_buff.ack != NOT_AN_ACK) {
                ...
                rcv_list2(sockfd, serv_addr, len, temp_buff, win_buf_snd, win_buf_rcv, seq_to_send, W, pkt_fly,&list,
                         dimension, loss_prob, window_base_snd, window_base_rcv, byte_written,shm_snd);
                printf("dimension %d\n",dimension);
